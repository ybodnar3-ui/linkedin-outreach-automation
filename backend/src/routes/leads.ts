...
import { getLeadEvents } from '../services/events';
...
// Server-side Sales Navigator / LinkedIn-search scraping required Playwright and
// was removed (ADR-001: extension-only). Lead search/import now happens in the
// Chrome extension popup (web search + Voyager), which POSTs to
// /api/extension/import-leads. These endpoints return 501 so the dashboard shows
// a clear message instead of silently failing.
router.post('/import-sales-nav', (_req: Request, res: Response) => {
  return res.status(501).json({
    error: 'Server-side scraping was removed. Import leads from the extension popup (Search / Batch search) instead.',
  });
});

router.post('/import-linkedin-search', (_req: Request, res: Response) => {
  return res.status(501).json({
    error: 'Server-side scraping was removed. Import leads from the extension popup (Search / Batch search) instead.',
  });
});

// GET /api/leads/:id/events — append-only event history for a lead
router.get('/:id/events', (req: Request, res: Response) => {
  const lead = db.prepare('SELECT id FROM leads WHERE id = ?').get(req.params.id) as { id: string } | undefined;
  if (!lead) return res.status(404).json({ error: 'Lead not found' });
  const events = getLeadEvents(req.params.id, 200).map(e => ({
    ...e,
